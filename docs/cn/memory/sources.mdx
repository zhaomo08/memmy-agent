---
title: Agent 来源与扫描
description: 了解 Memmy 如何扫描八种内置 Agent 的本地历史，以及安装的 Hook、插件、Skill 和实时记忆效果。
icon: ScanSearch
---
...
| ChatWise | Agent Skill | `~/.agents/skills/memmy-memory` | 在 ChatWise Agent 模式任务中搜索、读取和写入共享记忆 |
...
- **ChatWise 边界**：ChatWise 当前没有开放 Agent 模式的原生 Hook 或 MCP 生命周期。Memmy 以只读方式扫描其 SQLite 历史，并安装 Skill 执行显式生命周期与查询命令；不会复制 Provider 配置或 API Key。
