...
description: Learn how Memmy scans local history from eight built-in Agents and which Hooks, plugins, Skills, and live-memory behaviors it installs.
icon: ScanSearch
---
...
| ChatWise | `~/Library/Application Support/app.chatwise/app.db` (macOS) | Agent Skill |
...
| ChatWise | Agent Skill | `~/.agents/skills/memmy-memory` | Searches, reads, and writes shared memory during ChatWise Agent-mode tasks |
...
- **ChatWise boundary:** ChatWise does not currently expose a native Hook or MCP lifecycle in Agent mode. Memmy reads its SQLite history in read-only mode and installs the Skill for explicit lifecycle/search commands; it never copies provider configuration or API keys.
