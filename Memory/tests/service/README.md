...
The `tests/service` tree groups `MemoryService` behavior tests by domain, and
the table also links the related REST contract suites. Test titles stay
unchanged during migration so failures remain searchable with `rg` and Vitest
`-t`.
...
| Retrieval     | L1 Trace/Span filtering | `retrieval/l1-trace-span-filter.test.ts`  | `L1 Trace/Span recall filtering`                  | parent Trace replacement and recalled Span limits                          | unit        |
...
| REST contract | Panel and viewer        | `../contract/rest-panel-events.test.ts`   | `REST panel contract`                             | viewer HTML, panel reads, search, detail, deletion                         | contract    |
...
npx vitest run tests/service/retrieval/l1-trace-span-filter.test.ts
...
npx vitest run tests/service/evolution/negative-experience.test.ts
npx vitest run tests/service/evolution/span-big-turn.test.ts
...
npx vitest run tests/contract/rest-panel-events.test.ts
